...
var ANNOTATION = '<edit data-key="%s" data-original="%s">%s</edit>';
...
exports.decorate = function (bundle) {
    ...
    var proto = {
        ...
        get: function (key, settings, cb) {
            var that, value;

            that = this;
            this.super_.get(key, settings, function (err, value) {
                if (err) {
                    return cb(err);
                }
                ...
                if (Array.isArray(value)) {
                    return cb(null, value.map(function (str, idx) {
                        return that._format(key + '[' + idx + ']', str);
                    }));

                } else if (typeof value === 'object') {
                    return cb(null, Object.keys(value).map(function (name) {
                ...
                return cb(null, that._format(key, value));
            });
        ...
        _format: function (key, str) {
            ...
            return util.format(ANNOTATION, key, orig, str);
        },
